...
class LIGController extends Controller
{
    public function create($id){
        ...
        return view('calculator.lig.create',compact('id','labs','parent','assets'));
    }
...
//            return redirect()->back()->with('failed','Reference data is not available');
        ...
        foreach ($request->noofdiv as $item) {
            if (isset($request->mulitplying_factor)  and $request->multiplying_factor!=0){
                $noofdiv[] = $item*$request->mulitplying_factor;
            }else{
                $noofdiv[] = $item;
            }
        }
        ...
        for ($i = 0; $i < count($x1); $i++) {
            ...
            $item->x1 = $x1[$i];
            $item->x2 = $x2[$i];
            $item->noofdiv=$request->noofdiv[$i];
            $item->asset_id=$request->assets;
            $item->k_value=$request->k_value;
        ...
        $allentries=Ligentries::where('parent_id',$entries->id)->get();
        $procedure = Procedure::find($job->item->capabilities->procedure);
        $uncertainties=explode(',',$procedure->uncertainties);
        $data=array();
        foreach ($allentries as $entry){
            ...
            $average_repeated_value=($x1+$x2)/$n;
            $reference=$average_repeated_value;
            $uuc=$entry->uuc;
...
//                dd($uncertainty_due_to_resolution_of_uuc);
            ...
            $uncertainty_due_to_spatial_in_homogeniety_of_bath_u5=0;
            $uncertainty_due_to_loading_effect_lig=0;
            $uncertainty_due_to_stability_with_time_u8=0;
            $uncertainty_due_to_parallex_of_indication_of_uuc_u10=0;

            if (in_array('uncertainty-due-to-spatial-in-homogeniety-of-bath-u5',$uncertainties)) {
                $uncertainty_due_to_spatial_in_homogeniety_of_bath_u5=50/1000/sqrt(3);
            }
            if (in_array('uncertainty-due-to-loading-effect-lig',$uncertainties)) {
                $uncertainty_due_to_loading_effect_lig=0/1000/sqrt(3);
            }
            if (in_array('uncertainty-due-to-stability-with-time-u8',$uncertainties)) {
                $uncertainty_due_to_stability_with_time_u8=20/1000/sqrt(3);
            }
            if (in_array('uncertainty-due-to-parallex-of-indication-of-uuc-u10',$uncertainties)) {
                $uncertainty_due_to_parallex_of_indication_of_uuc_u10=0;
            }
            ...
            $data[$entry->fixed_value]=[
                ...
                "uncertainty-due-to-spatial-in-homogeniety-of-bath-u5"=>$uncertainty_due_to_spatial_in_homogeniety_of_bath_u5,
                "uncertainty-due-to-loading-effect-lig"=>$uncertainty_due_to_loading_effect_lig,
                "uncertainty-due-to-stability-with-time-u8"=>$uncertainty_due_to_stability_with_time_u8,
                "uncertainty-due-to-parallex-of-indication-of-uuc-u10"=>$uncertainty_due_to_parallex_of_indication_of_uuc_u10,

                'combined-uncertainty'=>$combined_uncertainty,
                'expanded-uncertainty'=>$expanded_uncertainties,
            ];
            $save_data=Ligentries::find($entry->id);
