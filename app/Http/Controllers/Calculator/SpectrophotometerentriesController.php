...
use App\Models\Spectrophotometerentries;
use App\Models\Unit;
use Illuminate\Http\Request;

class SpectrophotometerentriesController extends Controller
{
    public function create($id){
        ...
        return view('calculator.spectro.create',compact('id','labs','parent','assets','units'));
    }
    public function store(Request $request){
        ...
        foreach ($request->w_uuc as $item) {
            $w_uuc[] = $item;
        }
        foreach ($request->filtertype as $item) {
            $filtertype[] = $item;
        }
        $units=[];
        ...
        for ($i = 0; $i < count($x1); $i++) {
            $item = new Spectrophotometerentries();
            $item->parent_id = $request->parent_id;
            $item->x1 = $x1[$i];
            $item->x2 = $x2[$i];
            $item->x3 = $x3[$i];
            $item->uuc_wavelength = $w_uuc[$i];
            $item->filtertype = $filtertype[$i];
        ...
        $allentries=Spectrophotometerentries::where('parent_id',$entries->id)->get();
