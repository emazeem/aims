    ...
    public function fetch(){
        ...
        return DataTables::of($data)
            ->addColumn('@', function ($data) {
                $groupedIDs=[];
                $groups=Capabilities::where('group_id','!=',0)->get();
                foreach ($groups as $group){
                    $groupedIDs[]=$group->id;
                }
                if (in_array($data->id,$groupedIDs)){
                    return "
                <div class='checkbox checkbox-fill d-inline'>
                     <input type='checkbox' data-id='".$data->id."' name='action[]' class='actions' id='actions".$data->id."' checked>
