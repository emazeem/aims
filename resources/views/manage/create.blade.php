...
@section('content')
    ...
    @endif
    <div class="row">
        <h2 class="border-bottom text-dark">Add Items for Job</h2>
            ...
            <form method="post" action="{{route('jobs.manage.store')}}">
                ...
                @foreach($items as $item)
                    ...
                    <div class="form-check border-bottom">
                        ...
                        <label class="form-check-label text-lg ml-2" for="{{$item->id}}">{{$item->capabilities->name .' -'.$item->quantity }}</label>
                    </div>
                        @else
                        <div class="form-check border-bottom">
                            <input type="checkbox" name="items[]" value="{{$item->id}}" class="form-check-input" id="{{$item->id}}" disabled checked>
                            <label class="form-check-label text-lg ml-2 text-muted" for="{{$item->id}}">{{$item->capabilities->name.' -'.$item->quantity }}</label>
                        </div>
                    @endif
                        @if ($errors->has('items'))
