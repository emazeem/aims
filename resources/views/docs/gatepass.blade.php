<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Delivery Note</title>
    <link rel="stylesheet" href="{{url('docs.css')}}">
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css">

</head>

<body>

<div class="container">

    <div class="col-12 font-style mt-2">
        <div class="row">
            <div class="col-2 text-center custom-border">
                <img src="{{url('/img/aims.png')}}" class="mt-2 ml-2" width="100">
            </div>
            <div class="col-7 border-left-right-0 custom-border" >
                <p class="text-center b font-14 mt-5">Gate Pass for Equipment Going Outside of AIMS Cal Lab</p>
            </div>
            <div class="col-3 row custom-border font-9 p-0">
                <p class="text-center font-11 col-12 my-1">DOC. # AIMS-BM-FRM-04,</p>
                <div class="col-12 custom-bottom-border"></div>
                <p class="text-center font-11 col-12 my-2">Issue Date : 06-10-2020</p>
                <div class="col-12 custom-bottom-border"></div>
                <p class="text-center font-11 col-12 mt-2 mb-1">
                    Issue # 01
                    <span class="px-4"></span>
                    Rev # 02
                </p>
            </div>

        </div>
        <div class="row py-3">
            <div class="col-6 my-1 font-11 ">Quote Ref :<span class="custom-bottom-border " style="padding-left: 80%"></span></div>
            <div class="col-6 my-1 font-11 ">Address ; <span class="custom-bottom-border " style="padding-left: 70%"></span></div>
            <div class="col-6 my-1 font-11 ">Job Number :<span class="custom-bottom-border " style="padding-left: 70%"></span></div>
            <div class="col-3 my-1 font-11 ">Date From:
                <span class="custom-bottom-border " style="padding-left: 82%"></span>
            </div>
            <div class="col-3 my-1 font-11 ">Date To:
                <span class="custom-bottom-border " style="padding-left: 82%"></span>
            </div>
            <div class="col-6 my-1 font-11 ">
                Returnable <input type="checkbox" checked>
            </div>
            <div class="col-6 my-1 font-11 ">
                Nonreturnable <input type="checkbox">
            </div>

        </div>
        <div class="col-12 text-center">
            <p class=" font-14 b mt-3">Equipment Description
            </p>
        </div>
        <div class="row">
            <table class="table table-bordered">
                <thead>
                <tr>
                    <th class="text-xs">Sr#</th>
                    <th class="text-xs">Asset ID</th>
                    <th class="text-xs">Asset Description</th>
                    <th class="text-xs">Make</th>
                    <th class="text-xs">Model</th>
                    <th class="text-xs">Received By</th>
                    <th class="text-xs">Handed Over By</th>
                    <th class="text-xs">Function Check Value</th>
                    <th class="text-xs">Status Before Dispatch</th>
                    <th class="text-xs">Function Check Value</th>
                    <th class="text-xs">Status After Dispatch</th>
                    <th class="text-xs">Function Checked By</th>
                </tr>
                </thead>
                <tbody>
                <tr>
                    <td class="font-11">1</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                </tr>
                <tr>
                    <td class="font-10">2</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                </tr>
                <tr>
                    <td class="font-10">3</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                </tr>
                <tr>
                    <td class="font-10">4</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                </tr>
                <tr>
                    <td class="font-10">5</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                    <td class="font-10">-</td>
                </tr>

                </tbody>
            </table>
        </div>


        <div class="row text-center">
            <p class="col-12 mb-5 custom-border font-11">This document is the property of AIMS Cal Lab. It is not to be retransmitted, printed or copied without prior written permission of the company.</p>
        </div>


    </div>
</div>
</body>
</html>