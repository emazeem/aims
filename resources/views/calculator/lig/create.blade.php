...
@section('content')
    ...
    <div class="col-12">
        ...
        <h4><i class="fa fa-calculator"></i> Liquid In Glass Thermometer Calculator</h4>
    ...
    <div class="col-12">
        <form class="form-horizontal" action="{{route('lig.calculator.data.entry.store')}}" method="post">
            @csrf
            ...
            <div class="form-group col-md-6">
                <label for="k_value" class="control-label">K-value</label>
                <select class="form-control" id="k_value" name="k_value">
                    <option selected disabled>Select K-value</option>
                    <option value="mercury">Mercury</option>
                    <option value="alcohol">Alcohol</option>
                </select>
                @if ($errors->has('k_value'))
                    <span class="text-danger"><strong>{{ $errors->first('k_value') }}</strong></span>
                @endif
            </div>

            <div class="container row cal-inputs">
                <table id="myTable" class=" table order-list">
                    <thead>
                    <tr>
                        <td>Temperature Measured by UUC</td>
                        <td>X<sub>1</sub></td>
                        <td>X<sub>2</sub></td>
                        <td>No. of Div</td>
                        <td>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="noofdiv[]" class="form-control "/>
                            @if ($errors->has('noofdiv'))
                                <span class="text-danger">
                                    <strong>{{ $errors->first('noofdiv') }}</strong>
                                </span>
                            @endif
                        </td>
                        <td>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $("#addrow").on("click", function () {
                ...
                cols += '<td><input type="text" class="form-control" id="noofdiv'+counter+'" name="noofdiv[]"/></td>';
