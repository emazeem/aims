...
@section('content')
    ...
    <div class="col-12">
        ...
        <form action="{{route('forms.update')}}" method="post" enctype="multipart/form-data">
            @csrf
            <input class="form-control" id="id" name="id" value="{{$edit->id}}" type="hidden">
            <div class="row pb-2 mt-3">
                <label for="name" class="col-sm-2 control-label">Name</label>
                <div class="col-sm-10">
                    <input class="form-control" id="name" name="name" placeholder="Name" value="{{$edit->name}}">
                    @if ($errors->has('name'))
                ...
                <div class="col-sm-10">
                    <div class="form-check form-check-inline" style="width: 100%">
                        <select class="form-control" id="sops" name="sops[]" multiple>
                            @foreach($sops as $sop)
                                <option value="{{$sop->id}}" {{(in_array($sop->id,explode(',',$edit->sops)))?'selected':''}}>{{$sop->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    @if ($errors->has('sops'))
