...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Edit Capabilities & Prices</h1>
    </div>

    <div class="row pb-3">
        <div class="col-12">

            <form class="form-horizontal" action="{{url('capabilities/update/'.$edit->id)}}" method="post" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="form-check form-check-inline" style="width: 100%">
                            <select class="form-control" id="category" name="category">
                                ...
                                @foreach($parameters as $parameter)
                                    <option value="{{$parameter->id}}" {{($parameter->id==$edit->parameter)?"selected":""}}>{{$parameter->name}}</option>
                                @endforeach
                        ...
                        <div class="form-check form-check-inline" style="width: 100%">
                            <select class="form-control" id="procedure" name="procedure">
                                ...
                                @foreach($procedures as $procedure)
                                    <option value="{{$procedure->id}}" {{($procedure->id==$edit->procedure)?"selected":""}}>{{$procedure->name}}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="col-12 form-check">
                    <div class="text-right">
                        <input type="checkbox" class="form-check-input" {{($edit->accredited=="yes"?"checked":"")}} name="accredited" id="accredited">
